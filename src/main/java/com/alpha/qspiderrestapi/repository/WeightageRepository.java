...
public interface WeightageRepository extends JpaRepository<Weightage, Long> {
	...
	@Query(value = "select MAX(jspiders) as max_jspider,\r\n" + "MAX(qspiders) as max_qspider, \r\n"
			+ "MAX(pyspiders) as max_pyspider, \r\n" + "MAX(prospiders) as max_prospiders\r\n" + "from weightage\r\n"
	...
	@Transactional
	@Query(value = "SELECT dynamic_check_and_update(:qspiders,:jspiders, :pyspiders, :prospiders, :fieldName, :categoryId)", nativeQuery = true)
	void incrementWeightageValues(@Param(value = "qspiders") long qspiders, @Param(value = "jspiders") long jspiders,
			@Param(value = "pyspiders") long pyspiders, @Param(value = "prospiders") long prospiders,
			@Param(value = "fieldName") String fieldName, @Param(value = "categoryId") long categoryId);
